   ...
   char* b, unsigned int offset, unsigned int length) throw(IOException)
{
   ...
   else
   {
      ...
      rval = mSocket->receive(b, offset, length);
   }
   
   return rval;
}

int SocketInputStream::peek(char* b, unsigned int offset, unsigned int length)
throw(IOException)
{
   int rval = -1;
   ...
   if(length > mPeekLength)
   {
      ...
      // read from the socket until the peek buffer gets more data
      size_t offset = mPeekOffset + mPeekLength;
      int count = mSocket->receive(mPeekBuffer, offset, mPeekSize - offset);
      if(count > 0)
      {
