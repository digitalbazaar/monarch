...
static const short _daysInPreviousMonth[12] =
{
 ...
 */
inline static bool _isLeapYear(unsigned int year)
{
 ...
 */
// FIXME: why aren't we using strtol, etc?
inline static bool _stringToNumber(const char** s, int& num, int digits)
{
   bool rval = true;
         ...
         case '%':
            ...
            switch(*format)
            {
               ...
               case 'A':
                  ...
                  for(int i = 0; i < 12; ++i)
                  {
                     length = strlen(_daysOfWeek[i]);
                     if(strncasecmp(_daysOfWeek[i], s, length) == 0)
                     {
                        s += length;
                        parse = true;
                        break;
               ...
               case 'a':
                  ...
                  for(int i = 0; i < 12; ++i)
                  {
                     if(strncasecmp(_daysOfWeek[i], s, 3) == 0)
                     {
                        s += 3;
                        parse = true;
                        break;
               ...
               case 'B':
                  ...
                  for(int i = 0; i < 12; ++i)
                  {
                     length = strlen(_months[i]);
                     if(strncasecmp(_months[i], s, length) == 0)
                     {
                        s += length;
                        tm->tm_mon = i;
                        parse = true;
               ...
               case 'h':
                  ...
                  for(int i = 0; i < 12; ++i)
                  {
                     if(strncasecmp(_months[i], s, 3) == 0)
                     {
                        s += 3;
                        tm->tm_mon = i;
                        parse = true;
               ...
               case 'C':
                  ...
                  parse = _stringToNumber(&s, century, 2);
                  break;
               case 'd':
                  // day in month (2 digits)
                  old = s;
                  _stringToNumber(&s, tm->tm_mday, 2);
                  parse = (s != old);
                  break;
               case 'D':
               ...
               case 'H':
                  // hour in day (0-23)
                  old = s;
                  _stringToNumber(&s, tm->tm_hour, 2);
                  parse = (s != old);
                  break;
               case 'l':
               ...
               case 'm':
                  ...
                  old = s;
                  _stringToNumber(&s, tm->tm_mon, 2);
                  parse = (s != old);
                  ++tm->tm_mon;
               case 'M':
                  // minute in hour
                  old = s;
                  _stringToNumber(&s, tm->tm_min, 2);
                  parse = (s != old);
                  break;
               case 'n':
               case 't':
               ...
               case 'Y':
                  // year (4 digit)
                  parse = _stringToNumber(&s, tm->tm_year, 4);
                  tm->tm_year -= 1900;
                  break;
               case 'y':
                  // year (2 digit)
                  parse = _stringToNumber(&s, tm->tm_year, 2);
